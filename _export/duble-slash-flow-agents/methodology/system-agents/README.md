...
- **Teach once** (from brainstorm item H *(project-internal reference; not bundled)*). First time the user flags a new pattern ("also redact our internal project codename"), Redaction learns it for future sessions.
...
- Product brief (source of agent list + milestones) → `planning/briefs/brief-collab-capture-layer.md` *(project-internal reference; not bundled)*
- Technical stack → `planning/research/technical/technical-research-duble-slash-stack.md` *(project-internal reference; not bundled)*
